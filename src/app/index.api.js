...
// ================  mock data api  =====================
        ...
        api.testExecutions = $resource(api.baseUrl + 'executions.json');

        api.testInfo = $resource(api.baseUrl + 'test-info.json');

        api.userInfo = $resource(api.baseUrl + 'auth/users.json');

        api.testPlan = {
            list: $resource(api.baseUrl + 'test-plans.json')
        };
        api.adminInfo = {

          user: $resource(api.baseUrl + 'users.json'),
          box : $resource(api.baseUrl + 'boxes.json'),
          company: $resource(api.baseUrl + 'companies.json')

        };
// ================  server api  =========================
        // 以后加上companyId
        // 部署时
        // api.resourceBaseUrl = 'http://hcp.sysu.edu.cn/stage/v3/'
        // 开发时
        api.resourceBaseUrl = '/'

        api.retrieveBoxes = $resource(api.resourceBaseUrl + 'api/boxes');

        api.retrieveOneBox = $resource(api.resourceBaseUrl + 'api/boxes/:boxId')

        api.retrievePlans = $resource(api.resourceBaseUrl + 'api/plans')

        api.retrieveOnePlan = $resource(api.resourceBaseUrl + 'api/plans/:planId')

        api.retrieveOneExecution = $resource(api.resourceBaseUrl + 'api/executions/:executionId')

        api.createPlan = function(params) {
            return $resource(api.resourceBaseUrl + 'api/plans').save(params).$promise;
        }

        api.demoUpload = function(params) {
            return $resource(api.resourceBaseUrl + 'api/demoUpload').save(params).$promise;
        }

        api.auth = {

          isRegistedUser: function(params) {
            return $resource(api.resourceBaseUrl + 'api/login').save(params).$promise
          ...
          logout: function(params) {
            return $resource(api.resourceBaseUrl + 'api/logout').save(params).$promise
              .then(function(result) {
                  return Promise.resolve(result);
              })
          },
          ...
          isLogined: function() {
            return $resource(api.resourceBaseUrl + 'api/isLogined').get().$promise
              .then(function(result) {
